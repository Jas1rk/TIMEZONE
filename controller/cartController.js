  ...
  try {
    ...
    if (cartData) {
      ...
      if (existingProduct) {
        ...
        const ucData = await Cart.findOneAndUpdate(
          { user: userData._id },
          {
            $push: {
              products: {
...
const quantityIncrement = async (req, res) => {
  try {
    ...
    const { id } = req.body;
    const findQuantity = await Cart.findOne({
      user: user,
      "products.productId": id,
    });
...
const quantityDecrement = async (req, res) => {
  try {
    ...
    const quantityData = await Cart.findOne({
      user: user,
      "products.productId": id,
    });
    const price = productData.offprice;
...
const cartDelete = async (req, res) => {
  try {
    ...
    if (findCart) {
      const productTodelete = findCart.products.find(
